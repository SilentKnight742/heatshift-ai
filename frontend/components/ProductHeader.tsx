...
export default function ProductHeader({ consoleMode = false }: { consoleMode?: boolean }) {
  return (
    <header className={`product-header${consoleMode ? " console-header" : ""}`}>
      ...
      <nav aria-label="Primary navigation"><Link href="/#worked-example">Example</Link><Link href="/#method">How it works</Link><Link href="/#research">Research</Link><Link className={consoleMode ? "nav-console active" : "nav-console"} href="/console">Console <span>↗</span></Link></nav>
    </header>
  );
}
