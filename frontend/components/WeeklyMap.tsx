...
function SvgFallback(props: WeeklyMapProps & { boundary: FeatureCollection<Geometry> }) {
  ...
  return <svg className="weekly-map-svg" viewBox="0 0 1000 560" role="img" aria-label={props.mode === "portfolio" ? `${props.stateCode} site portfolio map fallback` : `${props.selectedSite.name} thermal field fallback`} onDoubleClick={(event) => {
    ...
    {props.mode === "site" && <>
      ...
      {props.crews.map((crew, index) => { const dense = props.jobs.length > 16; const angle = index / Math.max(props.crews.length, 1) * Math.PI * 2; const [baseX, baseY] = project(props.selectedSite.centroid.longitude, props.selectedSite.centroid.latitude); const x = baseX + Math.cos(angle) * 45; const y = baseY + Math.sin(angle) * 45; return <g key={crew.crew_id} role="button" aria-label={crew.name} tabIndex={0} className="map-crew-marker" onClick={() => props.onSelectCrew(crew.crew_id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") props.onSelectCrew(crew.crew_id); }}><title>{crew.name}</title><circle cx={x} cy={y} r="22" fill="transparent" /><circle cx={x} cy={y} r={dense ? "9" : "12"} fill="#75dab2" stroke="#10251d" strokeWidth={dense ? "4" : "5"} />{!dense && <text x={x + 17} y={y + 4}>{crew.name}</text>}</g>; })}
      {props.jobs.map((job) => { const dense = props.jobs.length > 16; const [x, y] = project(job.location.longitude, job.location.latitude); return <g key={job.job_id} className="map-job-marker" role="button" aria-label={job.name} tabIndex={0} onClick={() => props.onSelectJob(job.job_id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") props.onSelectJob(job.job_id); }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { const crewId = event.dataTransfer.getData("text/crew-id"); if (crewId) props.onAssignCrew(job.job_id, crewId); }}><title>{job.name}</title><circle cx={x} cy={y} r="20" fill="transparent" /><circle cx={x} cy={y} r={dense ? "10" : "14"} fill="#fff" stroke="#172922" strokeWidth={dense ? "5" : "7"} /><circle cx={x} cy={y} r={dense ? "5" : "7"} fill={job.status === "completed" ? "#55d0a4" : job.status === "cancelled" ? "#8c9892" : "#ff8057"} />{!dense && <text x={x + 19} y={y + 4}>{job.name}</text>}</g>; })}
    </>}
...
export default function WeeklyMap(props: WeeklyMapProps) {
  ...
  useEffect(() => {
    ...
    void import("maplibre-gl").then((module) => {
      ...
      props.jobs.forEach((job) => {
        const button = document.createElement("button"); button.className = `weekly-map-marker status-${job.status}`; button.type = "button"; button.title = props.readOnly ? job.name : `${job.name} · drop an eligible crew here to assign`; button.setAttribute("aria-label", `Open ${job.name}`); button.addEventListener("click", () => props.onSelectJob(job.job_id)); if (!props.readOnly) { button.addEventListener("dragover", (event) => event.preventDefault()); button.addEventListener("drop", (event) => { event.preventDefault(); const crewId = event.dataTransfer?.getData("text/crew-id"); if (crewId) props.onAssignCrew(job.job_id, crewId); }); }
  ...
  return <section className="weekly-map-card" aria-label="Interactive operations map">
    ...
    <div className="weekly-map-foot"><span><i className="legend-cool" /> Cooler / lower burden</span><span><i className="legend-hot" /> Hotter / higher burden</span><span>{props.mode === "portfolio" ? "Select a site · double-click to start a new site" : props.readOnly ? "Click cells, jobs, or crews for details · estimates are labeled" : "Click buildings/jobs/crews · drag eligible jobs · estimates are labeled"}</span></div>
  </section>;
}
